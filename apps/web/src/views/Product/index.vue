...
const handleAIAction = (action: any) => {
	console.log("处理 AI Action:", action);
	switch (action.type) {
		case "NAVIGATE":
			if (action.path) {
			...
			break;
		case "OPEN_MODAL":
			console.log("打开模态框:", action.modalId);
			break;
		case "FILL_FORM":
			console.log("填充表单:", action.formData);
			break;
		default:
			console.warn("未处理的 Action 类型:", action.type);
	}
};

onMounted(() => {
	eventBus.on("action", handleAIAction);
});

onUnmounted(() => {
	eventBus.off("action", handleAIAction);
});
</script>
