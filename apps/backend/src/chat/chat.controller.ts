...
export class ChatController {
  ...
  @Get('product')
  async chatProduct(
    @Query() createChatDto: CreateChatDto,
    @Res() res: Response,
  ) {
    res.setHeader('Content-Type', 'text/event-stream; charset=utf-8');
    res.setHeader('Cache-Control', 'no-cache');
    ...
    const generator = this.chatService.streamProductChat(
      createChatDto.query,
      createChatDto.thread_id,
    );
  ...
  async chatPlaywright(
    ...
    const generator = this.chatService.streamPlaywrightChat(
      createChatDto.query,
      createChatDto.thread_id,
    );
  ...
  async chatDeepAgent(
    ...
    const generator = this.chatService.streamDeepAgentChat(
      createChatDto.query,
      createChatDto.thread_id,
    );
