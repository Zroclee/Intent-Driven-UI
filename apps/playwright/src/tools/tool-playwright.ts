...
export const extractPageStateTool = tool(
  async () => {
    ...
    const elementsMetadata: InteractiveElement[] = await p.evaluate(() => {
      ...
      const selectors = ['a[href]', 'button', 'input', 'select', 'textarea', '[role=\"button\"]', '[role=\"link\"]', '[role=\"checkbox\"]'];
      ...
      elements.forEach((el) => {
        ...
        if (rect.width > 0 && rect.height > 0 && rect.top >= 0 && rect.top <= window.innerHeight) {
          ...
          metadata.push({
            ...
            text: ((el.textContent || (el as HTMLInputElement).placeholder || "") as string).trim().substring(0, 50),
  ...
  },
  {
    ...
    schema: z.object({
      ...
      actions: z.array(z.object({
        ...
        type: z.enum(['click', 'fill', 'press', 'hover', 'check', 'selectOption', 'wait', 'goto']).describe("动作类型"),
        target: z.object({
          type: z.enum(['markId', 'css', 'role', 'text']),
          value: z.string(),
          name: z.string().optional(),
        }).optional().describe("定位器"),
        payload: z.object({
 ...
 */
export const browserTools = [ gotoUrlTool, extractPageStateTool, executePlaywrightActionsTool];
